...
# Jupyter Notebook VCS Hygiene & Environments (`notebook-sanitizer`)
...
- Invoking the slash command: `/rw-notebook-sanitizer`.
...
4. **VCS Hooks Compatibility:** Verify if other agents have set up pre-commit configurations (such as Husky by `vcs-workflow.agent`).
...
- **Headless Mode Override:** Refer to Phase 3 of [Headless Mode Override Protocol](../../references/headless-override.md) (writing observations to `<reportRoot>/.repo-wizard/reports/<repo-name-here>/agents/<repo-name-here>-observations-notebook-sanitizer-agent.md`).
...
## Red Flags
- Overwriting an existing `.pre-commit-config.yaml` or Husky shell script created by `vcs-workflow.agent` without merging configurations, breaking lint gates.
