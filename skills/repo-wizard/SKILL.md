...
2. **Unified Pre-Scan Analysis**: Before launching the orchestrator or questionnaire, you MUST execute the unified scan script: `node scripts/initial-codebase-scan.js --report-path <resolved_report_path>` (forwarding `--headless` if running in headless mode, and forwarding any `--pillar` flags if specified). You must verify the command exited with `0` (success) before proceeding:
...
1. **Local Interactive Mode**: Finish the interview first, deduplicate candidates. If running inside the Antigravity chat session, natively define and invoke the specialist subagents in parallel using the `define_subagent` and `invoke_subagent` tools (first loading metadata from `agent-registry.json` and passing prompt contents from `agents/<subagent-name>.md` to `define_subagent` to enable direct workspace tools, then calling `invoke_subagent`), bypassing the CLI runner. You MUST apply the **Pillar Concurrency & Batching Rule** (see **Specialist Quality Pillars & Concurrency Framework** above) to partition subagents into batches of at most 6 total, waiting for the active batch to complete before starting the next batch. Run verification and VCS rollback on failure.
...
3. **Execution & Synchronization Rules**:
   ...
   - **Enforce Explicit Parameters**: When spawning `run-fallback-sequential-orchestration.js` (CLI Fallback Mode Only), you MUST explicitly pass the configured parameters (such as `--report-path <reportRoot>`, `--report-style <reportStyle>`, `--mock-cli <isMock>`, `--redact`, and any `--pillar` flags if configured).
