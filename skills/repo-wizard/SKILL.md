...
3. **Execution & Synchronization Rules**:
   ...
   - **Sync Gate**: You MUST wait for all background specialist subagents to complete their scans, report back, and write their observations and contract files to disk before executing the compilation utility (`node scripts/repo-wizard.js compile`). If you compile before they finish, Section 4 (Detailed Quality Pillars Analysis) of the Full Technical Report will be blank.
