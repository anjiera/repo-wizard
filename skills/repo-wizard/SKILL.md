...
3. **Execution & Synchronization Rules**:
   ...
   - **Enforce Explicit Parameters**: When spawning `run-orchestration.js`, you MUST explicitly pass the `--target-path <targetPath>` parameter (and other parameters like `--report-path <reportRoot>` and `--redact <isRedact>` if configured).
