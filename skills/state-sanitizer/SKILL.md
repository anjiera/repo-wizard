...
# State Sanitization Auditing & Scaffolding (`state-sanitizer`)
...
- Invoking the slash command: `/rw-state-sanitizer`.
...
- **Headless Mode Override:** If `MODE=HEADLESS_REMOTE` or `MODE=HEADLESS_LOCAL` is active, do not perform file writes. Instead, write suggested additions directly into the generated markdown report Observations file at `.repo-wizard/reports/<repo-name-here>/agents/<repo-name-here>-observations-state-sanitizer-agent.md`.
