...
### Agent-to-Agent (A2A)
A design pattern where multiple specialized AI agents communicate and coordinate with each other to solve a complex task. In `repo-wizard`, a lead **Orchestrator** agent dispatches specific auditing subtasks to specialist subagents (e.g., the `privacy-hardener`).
