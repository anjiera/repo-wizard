...
If the `tool-scaffolder` detects a version mismatch between the saved JSON contract and its active execution schema, it will halt execution, notify the developer of the format incompatibility, and request a fresh scan to update the contracts.
