...
16. **Emoji Restrictions**: Under no circumstances should agent responses, descriptions, or outputs contain unapproved emojis. Emojis are disallowed except for approved status color circles (🟢, 🔵, ⚪, 🟡, 🔴, ⚫) and terminal checkmarks (✓, ✗, ⚠) to maintain a professional, clean UI.
17. **Standard Codebase Sizing:** Sizing operations (estimating lines of code, counting files, detecting language profiles) must use the `solo-dev-toolkit/scripts/count-loc.js` utility rather than arbitrary PowerShell/Bash expressions.
