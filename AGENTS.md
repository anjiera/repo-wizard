...
8. **Active Workspace Execution Enforced:** The CLI orchestrator (`run-adk-orchestrator.js`) and scanner scripts operate strictly on the active local workspace directory (`process.cwd()`). Command-line parameters for alternative local target paths or positional arguments are not supported, ensuring that the tool is always run from within the repository codebase being analyzed. All invoking agents and code reviewers must respect this explicit contract.
...
11. **Banned Sandbox Bypass and Context-Bridging:** You are strictly forbidden from writing, generating, or committing custom helper scripts to serialize, copy, or bundle target repository files into JSON or any other temporary workspace format. Furthermore, the Lead Agent MUST NEVER act as a context or code/metadata bridge (such as copying code files, file lists, directory paths, or sizing summaries) to subagents via `send_message`. All specialist subagents possess full read/write access to the active workspace directory and MUST read and analyze target files directly via their own absolute path file-viewing tools to prevent token waste.
12. **Atomic Commits Mandated:** When generating or executing an implementation plan, you must always structure and execute your work as atomic, logical commits where possible. Avoid monolithic commits spanning unrelated features or containing thousands of lines of changes. Each discrete task should be committed independently.
