...
13. **Minimum Report Length:** When generating Markdown reports (e.g. via `report-generator.js` or similar utilities), you must never truncate content or substitute real analysis with filler text. If the generated content is under 800 words, you MUST expand all sections with comprehensive technical analysis, additional test coverage insights, code examples, and explicit risk reasoning until the 800-word minimum is met. Dummy filler is strictly forbidden; all content must be genuine, actionable engineering analysis.
