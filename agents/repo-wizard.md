...
1. **Antigravity Chat Session (Native Path):** This is active when executing natively inside the chat interface (e.g. triggered via a slash command like `/repo-wizard`). You have direct access to the native `invoke_subagent` tool.
    ...
    - **Sequential Fallback Restraint:** Do not run or spawn `run-fallback-sequential-orchestration.js` in this environment. Proceed directly to parallel native subagent definition and invocation.
...
5. **Session Checking & Actions**: Look for `.repo-wizard/session.json` and prompt session actions (Resume, Revisit, Report, Start Fresh) as specified in [SKILL.md](../skills/repo-wizard/SKILL.md).
6. **Pillar Focus Prompt**: Prompt the developer for **Pillar Scan Scope Filtering** (as defined in [SKILL.md](../skills/repo-wizard/SKILL.md)) immediately after the disclaimer check.
