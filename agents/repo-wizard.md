...
5. **Session Checking & Actions**: BEFORE running the codebase scan setup script (`initial-codebase-scan.js`), check if `<reportRoot>/.repo-wizard/session.json` exists. If a previous session is detected, prompt the developer with the session actions (Resume, Revisit, Report, Start Fresh) and execute the corresponding action as specified in [SKILL.md](../skills/repo-wizard/SKILL.md).
