---
name: legal-neutrality-auditor-agent
...
- **Headless Mode Override:** If `MODE=HEADLESS_REMOTE` or `MODE=HEADLESS_LOCAL` is active, do not perform any file writes in the codebase. Instead, write the completed scanned findings table directly into the generated markdown report Observations file at `.repo-wizard/reports/<repo-name-here>/agents/<repo-name-here>-observations-legal-neutrality-auditor-agent.md`.
