...
- **Headless Mode Override:** Refer to Step 3 of [Headless Mode Override Protocol](../references/headless-override.md) (writing observations to `<reportRoot>/.repo-wizard/reports/<repo-name-here>/agents/<repo-name-here>-observations-agent-alignment-auditor.md`).

Coordinate with the `tooling-engineer.agent` to deploy agent validation and evaluation controls, adhering to the following rules:
...
1. **Evaluation Framework Scaffolding:** Scaffold rubric-based evaluation suites (`evals/<agent-name>.js`) and LLM-as-a-Judge runner scripts (`run-evals.js`) to systematically test prompts under varied inputs.
